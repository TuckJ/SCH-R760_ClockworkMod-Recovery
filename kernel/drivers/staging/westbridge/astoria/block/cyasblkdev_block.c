	...
	if (dev_data.desc_p.locked) {
		...
		if (ret != CY_AS_ERROR_SUCCESS) {
			...
			cy_as_hal_print_message(KERN_ERR"%s cannot release"
				" storage\n", __func__) ;
			#endif
		cy_as_hal_print_message("leaving cyasblkdev_add_disks since its locked\n");
	...
	if (bd) {
		...
		for (bus_num = 0; bus_num <= 1; bus_num++) {
			...
			if (stat == CY_AS_ERROR_SUCCESS) {
				...
				if( stat == CY_AS_ERROR_SUCCESS)
				{
				cy_as_hal_print_message("media count is incremented\n");
					check_media_count++;
				}
...
void  cyasblkdev_blk_exit(void)
{
	...
	cy_as_hal_print_message (KERN_ERR"[%s] dev = %d ", __func__, g_is_block_dev_ready) ; 
